...
static int __init mod_test_init(void) {
  ...
  major_num = register_chrdev(0, "mod_test", &file_ops);
  if (major_num < 0) {
    printk(KERN_ALERT "Could not register device : % d\n", major_num);
    return major_num;
  } else {
    printk(KERN_INFO "mod_test module loaded with device major number % d\n",
           major_num);
    return 0;
  }
}

static void __exit mod_test_exit(void) {
